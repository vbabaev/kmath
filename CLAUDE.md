...
- **Production stack** lives in the sibling [`../infra/`](../infra/) repo: `docker-compose.yml` (pins `name: klearn`; references `../klearn/backend` and `../klearn/dist`), `Caddyfile` (serves the klearn dist + reverse-proxies `/api` to `backend:3000`, auto-TLS for `klearn.vbabaev.uk`), and `deploy/` (Ansible). `backend/Dockerfile` (Node 22 alpine) lives here in klearn because it's coupled to the backend source. **Deploy:** from `vibe/infra/`, run `ansible-playbook -i deploy/inventory.yml deploy/deploy.yml`. The playbook runs backend tests + `npm run build` in klearn locally, rsyncs `vibe/klearn/` → `/opt/klearn/klearn/` and `vibe/infra/` → `/opt/klearn/infra/`, then `docker compose up -d --build` from `/opt/klearn/infra/`. `klearn/backend/.env` is root-owned on the host and never overwritten by deploys — update it manually over SSH when a new env var is needed.
