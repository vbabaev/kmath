...
- **Backend cutover in progress** — see `docs/backend-design.md`. Phase 0–5 landed: `backend/` (Express + Mongo) exposes the API behind Google OAuth + an email allowlist. `GET /api/profiles`, `GET /api/profiles/:id`, `PUT /api/profiles/:id` enforce role-based authz (teacher sees all, student sees only own; 401/403 on mismatch). Auth routes: `/api/auth/google[/callback]`, `/api/auth/me`, `/api/auth/config`, `/api/auth/dev-login` (gated by `ALLOW_DEV_LOGIN=true` for local), `/api/auth/logout`. Sessions persist in the `sessions` Mongo collection via connect-mongo so deploys don't log everyone out. The email→profileId+role mapping lives in `backend/auth-allowlist.json` (gitignored, deploy-time secret). Frontend has a Login screen (`src/screens/Login.jsx`) that calls `/api/auth/config` to know which methods are available; on mount App.jsx hits `/api/auth/me`, routes to login if 401, otherwise pulls profiles and routes. `src/api.js` sends `credentials: 'include'`. Quiz auto-save is debounced 750 ms and abortable on unmount. Vite dev proxy forwards `/api/*` → `http://localhost:3000`. Production stack lives at the repo root: `backend/Dockerfile` (Node 22 alpine), `Caddyfile` (serves `./dist` + reverse-proxies `/api` to `backend:3000`, auto-TLS for `klearn.vbabaev.uk`), and `docker-compose.yml` (three containers on an internal network with named volumes). Deploy = `npm ci && npm run build` to populate `./dist`, then `docker compose up -d --build`.
