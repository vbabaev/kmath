...
- **Backend cutover in progress** — see `docs/backend-design.md`. Phase 0–3 landed: `backend/` (Express + Mongo) exposes `GET /api/profiles`, `GET /api/profiles/:id`, `PUT /api/profiles/:id` with append-only sessions and immutable-field package checks; `npm run seed` and `npm run import-backup` available; `docker-compose.dev.yml` runs Mongo for local dev. The frontend talks to the API (`src/api.js`, `src/profiles.js` rewritten async, `src/settings.js` reduced to `kmath.profileId` only); App.jsx holds an `allProfiles` list as the source of truth, derives `activeProfile` via useMemo, and splices server responses back into the list after each mutation. Quiz auto-save is debounced 750 ms and abortable on unmount. Vite dev proxy forwards `/api/*` → `http://localhost:3000`. Production stack lives at the repo root: `backend/Dockerfile` (Node 22 alpine), `Caddyfile` (serves `./dist` + reverse-proxies `/api` to `backend:3000`, auto-TLS), and `docker-compose.yml` (three containers on an internal network with named volumes). Deploy = `npm ci && npm run build` to populate `./dist`, then `docker compose up -d --build`. No auth gate in the interim — Phase 5 adds Google OAuth + allowlist.
