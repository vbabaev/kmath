...
- **Extra Math** (`group: 'extra'`):
  ...
  - `negativeNumbers` 🌡️ ("Negative Numbers", sky/blue) — `{ left, op, right, answer, _key }` — add or subtract two integers in `[-50, 50]` with seven uniformly-picked templates covering every sign shape: `-a + b`, `a - b`, `-a - b`, `a + (-b)`, `a - (-b)`, `-a + (-b)`, `-a - (-b)`. Display uses the typographic minus (U+2212), parens around any negative right operand, positives in emerald and negatives in rose so the kid eyeballs the sign before doing math. Text input only (numeric); accepts ASCII `-`, U+2212, en dash; max 4 chars; own Check button (Quiz hides its global one whenever a module exposes a custom Input). `tips(problem)` returns 3 tips: (1) number-line orientation — adding moves right, subtracting moves left, (2) three coloured `FormulaCard`s for the universal rules (same-signs rose: `-3 - 15 = -(3 + 15) = -18`, different-signs sky: `-10 + 3 = -(10 - 3) = -7`, minus-a-minus violet: `8 - (-3) = 8 + 3 = 11`), and (3) a per-problem walkthrough that first normalises away a negative right operand and then applies the same/different-sign rule. Dedup `key` = `neg:<left>:<op>:<right>`.
  - `roman` 🏛️ ("Roman Numerals", amber/orange) — `{ kind, number, roman, answer, choices?, _key }` — 4 question kinds picked uniformly: **`numToRomanText`** (number 3–500 → type the Roman, custom Input with auto-uppercase + char whitelist + own Check Answer button since the global one is hidden when a module provides Input), **`numToRomanMC`** (number 200–2500 → pick the Roman from 4 options), **`romanToNumText`** (Roman 3–500 → type the number, numeric input), **`romanToNumMC`** (Roman 200–2500 → pick the number). MC range goes higher because phone-typing CMXLIX is hostile; text range stays small because a V/X slip should cost seconds, not minutes. Distractors are off-by-small-N numeric perturbations (±1, ±5, ±9, ±10, ±50, ±100) re-encoded — they share long prefixes with the correct Roman so the kid can't pick by shape. `tips(problem)` returns 3 tips: (1) the seven-symbol table, (2) the six subtractive pairs (IV / IX / XL / XC / CD / CM), and (3) the *current problem's* place-value decomposition (e.g. `247 = 200 + 40 + 7 = CC + XL + VII = CCXLVII`, or the reverse parse for romanToNum kinds). Dedup `key` = `roman:<kind>:<number>`.
